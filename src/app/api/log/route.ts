  ...
  try {
    ...
    if (!validation.success) {
      ...
      return NextResponse.json(
        { error: "Data validation failed", details: validation.error.errors },
        { status: 500 }
      );
    }
    ...
    const return_data = {
      average_mood: Math.round(
        validData.reduce((acc, curr) => acc + curr.mood, 0) / validData.length
      ),
      average_energy: Math.round(
        validData.reduce((acc, curr) => acc + curr.energy, 0) / validData.length
      ),
      entries: validData,
    };
